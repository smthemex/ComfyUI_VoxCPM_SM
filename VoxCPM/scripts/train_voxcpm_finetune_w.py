...
def train(
    ...
    vae_path="",
    # 新增参数：分步训练支持
    start_step: int = 0,      # 从哪个步数开始训练
    train_steps: int = 0,     # 训练多少步（0表示使用num_iters）
    # 新增参数：时间后缀
    use_timestamp_suffix: bool = False,  # 是否在保存路径中添加时间戳后缀

):
    _ = config_path

    params={"vae_path":vae_path,"ckpt_path":ckpt_path,}
    ...
    base_model = VoxCPMModel.from_local(pretrained_path, optimize=False, training=True, lora_config=LoRAConfig(**lora) if lora else None,**params)
    ...
    # 在Windows上，多进程数据加载器经常出现问题，所以设置num_workers=0
    # 特别是在ComfyUI的子进程中
    safe_num_workers = 0 if os.name == 'nt' else num_workers
    ...
    loaded_start_step = 0
    if accelerator.rank == 0:
        loaded_start_step = load_checkpoint(model, optimizer, scheduler, save_dir)
    # Broadcast start_step to all processes
    if hasattr(accelerator, 'all_reduce'):
        start_step_tensor = torch.tensor(loaded_start_step, device=accelerator.device)
        accelerator.all_reduce(start_step_tensor)
        loaded_start_step = int(start_step_tensor.item())
    
    # 使用传入的start_step参数（如果提供了）
    if start_step > 0:
        loaded_start_step = start_step
    
    if loaded_start_step > 0 and accelerator.rank == 0:
        tracker.print(f"Resuming training from step {loaded_start_step}")

    # 如果提供了train_steps，则调整num_iters
    if train_steps > 0:
        num_iters = loaded_start_step + train_steps
    
    # Resume tracker for signal handler to read current step
    resume = {"step": loaded_start_step}
    ...
    def _signal_handler(signum, frame, _model=model, _optim=optimizer, _sched=scheduler, _save_dir=save_dir, _pretrained=pretrained_path, _hf_id=hf_model_id, _dist=distribute, _resume=resume):
        try:
            cur_step = int(_resume.get("step", loaded_start_step))
        except Exception:
            cur_step = loaded_start_step
        print(f"Signal {signum} received. Saving checkpoint at step {cur_step} ...")
        try:
            save_checkpoint(_model, _optim, _sched, _save_dir, cur_step, _pretrained, _hf_id, _dist, use_timestamp_suffix)
    ...
    try:
        ...
        if threading.current_thread() is threading.main_thread():
            ...
            print("Note: Running in worker thread, signal handlers not available")
            print("Training will proceed but checkpoint won't be saved on interruption")
    except Exception as e:
    ...
    def get_next_batch():
        ...
        try:
            ...
            try:
                ...
                # If the dataset is empty, raise a more informative error
                raise RuntimeError(
                    "Training dataset is empty. This could be because: "
                    "1. The training manifest file is empty or doesn't contain valid data\n"
                    "2. All samples were filtered out by max_batch_tokens filtering\n"
                    "3. The dataset loading failed\n"
                    f"Original dataset had {num_train_samples} samples."
                )

    with tracker.live():
        for step in range(loaded_start_step, num_iters):
            ...
            if step % save_interval == 0 and accelerator.rank == 0:
                save_checkpoint(model, optimizer, scheduler, save_dir, step, pretrained_path, hf_model_id, distribute, use_timestamp_suffix)

    if accelerator.rank == 0:
        save_checkpoint(model, optimizer, scheduler, save_dir, num_iters, pretrained_path, hf_model_id, distribute, use_timestamp_suffix)
    if writer:
        writer.close()
...
def save_checkpoint(model, optimizer, scheduler, save_dir: Path, step: int, pretrained_path: str = None, hf_model_id: str = "", distribute: bool = False, use_timestamp_suffix: bool = False):
    """
    ...
    tag = "latest" if step == 0 else f"step_{step:07d}{timestamp_suffix}"
    folder = save_dir / tag
    folder.mkdir(parents=True, exist_ok=True)
    
    if use_timestamp_suffix and step > 0:
        print(f"Saving checkpoint with timestamp suffix: {timestamp_suffix}")
    ...
    # Update (or create) a `latest` folder pointing to the most recent checkpoint folder
    latest_link = save_dir / "latest"
    try:
        # 在Windows上，优先使用文件夹复制而不是符号链接
        ...
        print(f"Warning: failed to update latest checkpoint link at {latest_link}: {e}")


# if __name__ == "__main__":
#     from ..src.voxcpm.training.config import load_yaml_config
