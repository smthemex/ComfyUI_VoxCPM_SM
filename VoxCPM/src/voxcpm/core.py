...
class VoxCPM:
    def __init__(
        ...
        **kwargs,
        ...
        if arch == "voxcpm2":
            self.tts_model = VoxCPM2Model.from_local(voxcpm_model_path, optimize=optimize, device=device, lora_config=lora_config,**kwargs)
            print("Loaded VoxCPM2Model", file=sys.stderr)
        elif arch == "voxcpm":
            self.tts_model = VoxCPMModel.from_local(voxcpm_model_path, optimize=optimize, device=device, lora_config=lora_config,**kwargs)
        ...
        self.denoiser = None
        if not kwargs.get("load_denoiser",True):
